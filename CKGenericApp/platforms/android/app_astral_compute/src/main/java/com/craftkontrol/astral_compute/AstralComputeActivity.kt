...
fun AstralComputeScreen(modifier: Modifier = Modifier) {
    ...
    val permissionLauncher = androidx.activity.compose.rememberLauncherForActivityResult(
        ...
        Log.i("CKAstral", "Permission result: $permissions")
        
        val allGranted = permissions.values.all { it }
        
        pendingPermissionRequest?.let { request ->
            if (allGranted) {
                Log.i("CKAstral", "Permissions granted, granting WebView request")
                request.grant(request.resources)
            } else {
                Log.w("CKAstral", "Permissions denied, denying WebView request")
                request.deny()
            }
    ...
    LaunchedEffect(Unit) {
        ...
        webView.webChromeClient = SharedWebViewHelper.buildChromeClient(
            ...
            onPermissionRequest = { permissions, request ->
                Log.i("CKAstral", "Permission callback: ${permissions.joinToString()}")
