package com.craftkontrol.meteo
...
@Composable
fun MeteoScreen(modifier: Modifier = Modifier) {
    ...
    val permissionLauncher = androidx.activity.compose.rememberLauncherForActivityResult(
        ...
        Log.i("CKMeteo", "Permission result: $permissions")
        
        val allGranted = permissions.values.all { it }
        
        pendingPermissionRequest?.let { request ->
            if (allGranted) {
                Log.i("CKMeteo", "Permissions granted, granting WebView request")
                request.grant(request.resources)
            } else {
                Log.w("CKMeteo", "Permissions denied, denying WebView request")
                request.deny()
            }
    ...
    LaunchedEffect(Unit) {
        ...
        webView.webChromeClient = SharedWebViewHelper.buildChromeClient(
            ...
            onPermissionRequest = { permissions, request ->
                Log.i("CKMeteo", "Permission callback: ${permissions.joinToString()}")
                pendingPermissionRequest = request
                permissionLauncher.launch(permissions)
            }
        )
        
        webView.loadUrl("https://craftkontrol.github.io/AI_Agents/MeteoAgregator/")
    }
