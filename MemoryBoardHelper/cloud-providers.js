/**
 * Cloud Provider Implementations (reduced)
...
class CKServerApiProvider extends CloudProvider {
    ...
    async authenticate() {
        ...
        try {
            // Quick status check to validate token/user
    ...
    buildPayload(data) {
        // Accept already-wrapped payloads or wrap raw data
...
class WebDAVProvider extends CloudProvider {
    ...
    async upload(data) {
        ...
        const payload = data && data.data ? data : {
            ...
            deviceId: (window.storageSyncEngine && window.storageSyncEngine.deviceId) || 'unknown',
            data
        };
